...
complete COMMAND/RESULT wire layout and protected application CAS API unfrozen.
ConfigStore v2 persistence/runtime details are governed separately by the
tokenized-layout design below and its implementation slice. Its focused
independent audit returned PASS WITH FIXES with no BLOCKER/HIGH. Final focused
verification returned PASS WITH MINOR DOC FIX; R1 is applied.
...
partition followed by a fresh v2 baseline; firmware must not silently erase
legacy/mixed/unknown evidence. Do not implement automatic v1 -> v2 migration unless a
